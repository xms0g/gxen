...
// - To use 16-bit mIndices + allow large mMeshes: backend need to set 'io.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset' and handle ImDrawCmd::VtxOffset (recommended).
// - To use 32-bit mIndices: override with '#define ImDrawIdx unsigned int' in your imconfig.h file.
#ifndef ImDrawIdx
...
namespace ImGui
{
    ...
    //   - For 'ImGuiKey key' you can still use your legacy native/user mIndices according to how your backend/engine stored them in io.KeysDown[].
    // With IMGUI_DISABLE_OBSOLETE_KEYIO: (this is the way forward)
    //   - Any use of 'ImGuiKey' will assert when key < 512 will be passed, previously reserved as native/user keys mIndices
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasVtxOffset  = 1 << 3,   // Backend Renderer supports ImDrawCmd::VtxOffset. This enables output of large mMeshes (64K+ mVertices) while still using 16-bit mIndices.
};
...
struct ImGuiStyle
{
    ...
    bool        AntiAliasedLinesUseTex;     // Enable anti-aliased lines/borders using mTextures where possible. Require backend to render with bilinear filtering (NOT point/nearest filtering). Latched at the beginning of the frame (copied to ImDrawList).
...
struct ImGuiIO
{
    ...
    IMGUI_API void  SetKeyEventNativeData(ImGuiKey key, int native_keycode, int native_scancode, int native_legacy_index = -1); // [Optional] Specify index for legacy <1.87 IsKeyXXX() functions with native mIndices + specify native keycode, scancode.
    ...
    // Legacy: before 1.87, we required backend to fill io.KeyMap[] (imgui->native map) during initialization and io.KeysDown[] (native mIndices) every frame.
    // This is still temporarily supported as a legacy feature. However the new preferred scheme is for backend to call io.AddKeyEvent().
    //   Old (<1.87):  ImGui::IsKeyPressed(ImGui::GetIO().KeyMap[ImGuiKey_Space]) --> New (1.87+) ImGui::IsKeyPressed(ImGuiKey_Space)
#ifndef IMGUI_DISABLE_OBSOLETE_KEYIO
    int         KeyMap[ImGuiKey_COUNT];             // [LEGACY] Input: map of mIndices into the KeysDown[512] entries array which represent your "native" keyboard state. The first 512 are now unused and should be kept zero. Legacy backend will write into KeyMap[] using ImGuiKey_ mIndices which are always >512.
...
// The maximum line width to bake anti-aliased mTextures for. Build atlas with ImFontAtlasFlags_NoBakedLines to disable baking.
...
//   this fields allow us to render mMeshes larger than 64K mVertices while keeping 16-bit mIndices.
...
struct ImDrawCmd
{
    ...
    unsigned int    VtxOffset;          // 4    // Start offset in vertex buffer. ImGuiBackendFlags_RendererHasVtxOffset: always 0, otherwise may be >0 to support mMeshes larger than 64K mVertices with 16-bit mIndices.
    unsigned int    IdxOffset;          // 4    // Start offset in index buffer.
    unsigned int    ElemCount;          // 4    // Number of mIndices (multiple of 3) to be rendered as triangles. Vertices are stored in the callee ImDrawList's vtx_buffer[] array, mIndices in idx_buffer[].
    ImDrawCallback  UserCallback;       // 4-8  // If != NULL, call the function instead of rendering the mVertices. clip_rect and texture_id will be set normally.
...
enum ImDrawListFlags_
{
    ...
    ImDrawListFlags_AntiAliasedLines        = 1 << 0,  // Enable anti-aliased lines/borders (*2 the number of triangles for 1.0f wide line or lines thin enough to be drawn using mTextures, otherwise *3 the number of triangles)
    ImDrawListFlags_AntiAliasedLinesUseTex  = 1 << 1,  // Enable anti-aliased lines/borders using mTextures when possible. Require backend to render with bilinear filtering (NOT point/nearest filtering).
    ImDrawListFlags_AntiAliasedFill         = 1 << 2,  // Enable anti-aliased edge around filled shapes (rounded rectangles, circles).
    ImDrawListFlags_AllowVtxOffset          = 1 << 3,  // Can emit 'VtxOffset > 0' to allow large mMeshes. Set when 'ImGuiBackendFlags_RendererHasVtxOffset' is enabled.
};
...
struct ImDrawList
{
    ...
    unsigned int            _VtxCurrentIdx;     // [Internal] generally == VtxBuffer.Size unless we are past 64K mVertices, in which case this gets reset to 0.
    ...
    //   In future versions we will use mTextures to provide cheaper and higher-quality circles.
    ...
    // - We render triangles (three mVertices)
...
enum ImFontAtlasFlags_
{
    ...
    ImFontAtlasFlags_NoBakedLines       = 1 << 2,   // Don't build thick line mTextures into the atlas (save a little texture memory, allow support for point/nearest filtering). The AntiAliasedLinesUseTex features uses them, otherwise they will be rendered using polygons (more expensive for CPU/GPU).
};

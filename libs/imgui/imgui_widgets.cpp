...
// Tip: use ImGui::PushID()/PopID() to push mIndices or pointers in the ID stack.
...
{
    // FIXME-OPT: we could pre-compute the mIndices to fasten this. But only 1 active combo means the waste is limited.
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // without any carriage return, which would makes ImFont::RenderText() reserve too many mVertices and probably crash. Avoid it altogether.
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        for (int n = 0; n < 6; n++)
        {
            ...
            // Paint colors over existing mVertices

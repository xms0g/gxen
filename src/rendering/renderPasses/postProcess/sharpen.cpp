...
Sharpen::Sharpen(const std::string& name, const bool enabled) : IPostEffect(name, enabled) {
	shader = std::make_unique<Shader>("models/quad.vert", "post-processing/kernel.frag");
	shader->activate();
	shader->setInt("screenTexture", 0);
}

uint32_t Sharpen::render(const uint32_t sceneTexture, const uint32_t VAO,
